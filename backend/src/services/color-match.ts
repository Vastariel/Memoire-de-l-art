...
async function analyzeThumb(imageBuffer: Buffer): Promise<{ rgb: [number, number, number]; variance: number }> {
  ...
  for (let iter = 0; iter < 8; iter++) {
    ...
    if (n2 > 0) { r2 = sr2 / n2; g2 = sg2 / n2; b2 = sb2 / n2; }
  }
  ...
  const rgb: [number, number, number] = n1 >= pixels / 2 ? [r1, g1, b1] : [r2, g2, b2];
...
export function colorDelta(a: [number, number, number], b: [number, number, number]): number {
...
export async function analyzePhoto(imageBuffer: Buffer, targetHex: string): Promise<ColorMatchResult> {
  ...
  const verdict: ColorMatchResult['verdict'] =
    deltaE <= env.COLOR_DELTA_PERFECT ? 'parfait'
    : deltaE <= env.COLOR_DELTA_ACCEPT ? 'correct'
    : 'libre';

  // Hue bonus 0..15, then gently dampened for flat "aplat" images (low variance).
  const hueBonus = Math.round(15 * Math.max(0, Math.min(1, (env.COLOR_DELTA_ACCEPT - deltaE) / env.COLOR_DELTA_ACCEPT)));
