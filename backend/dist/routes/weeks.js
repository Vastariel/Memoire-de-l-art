...
exports.weekRoutes = weekRoutes;
const db_1 = require("../services/db");
const cycle_1 = require("../services/cycle");
async function familiesAndVariants(artworkId) {
    const [families, variants] = await Promise.all([
        db_1.db.query(`SELECT key, day_ AS day, name_fr, name_en FROM color_families WHERE artwork_id = $1 ORDER BY day_`, [artworkId]),
        db_1.db.query(`SELECT key, family_key, name_fr, name_en, hex FROM color_variants WHERE artwork_id = $1`, [artworkId]),
    ]);
    return {
...
async function weekRoutes(app) {
    ...
    app.get('/current', async (req, reply) => {
        ...
        return reply.send({ artwork: (0, cycle_1.publicArtwork)(a, lang), ...fv, weekDay: (0, cycle_1.weekDay)() });
    });
    ...
    app.get('/current/bundle', async (req, reply) => {
        ...
        return reply.send({ artwork: (0, cycle_1.publicArtwork)(a, lang), ...fv, weekDay: (0, cycle_1.weekDay)(), revealObfuscated: reveal });
    });
}
