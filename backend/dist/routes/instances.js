...
const createSchema = zod_1.z.object({
    ...
    mode: zod_1.z.enum(['shared', 'separate']).default('shared'),
    solo: zod_1.z.boolean().default(false),
});
const joinSchema = zod_1.z.object({ code: zod_1.z.string().min(4).max(8).transform(s => s.toUpperCase()) });
async function instanceRoutes(app) {
    ...
    app.post('/', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        for (let i = 0; i < 10; i++) {
            code = genCode();
            const dup = await db_1.db.query('SELECT 1 FROM instances WHERE code = $1', [code]);
            if (dup.rows.length === 0)
                break;
        }
        const inst = await db_1.db.query(`INSERT INTO instances (code, name, mode, solo, owner_user_id)
       VALUES ($1, $2, $3, $4, $5) RETURNING id`, [code, body.name ?? null, body.mode, body.solo, userId]);
        const id = inst.rows[0].id;
        await db_1.db.query(`INSERT INTO instance_members (instance_id, user_id) VALUES ($1, $2)`, [id, userId]);
    ...
    app.post('/join', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const inst = await db_1.db.query(`SELECT id, name, mode, solo FROM instances WHERE code = $1`, [code]);
        if (inst.rows.length === 0)
            return reply.code(404).send({ error: 'Instance introuvable.' });
        const it = inst.rows[0];
        await db_1.db.query(`INSERT INTO instance_members (instance_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [it.id, userId]);
        await importSharedPhotos(userId, it.id, it.mode);
        const members = await db_1.db.query(`SELECT COUNT(*) AS n FROM instance_members WHERE instance_id = $1`, [it.id]);
        return reply.send({
    ...
    app.get('/mine', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const rows = await db_1.db.query(`WITH ranked AS (
       ...
       ORDER BY i.created_at`, [userId, year, week]);
        return reply.send({
    ...
    app.get('/:id/artwork', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const contribs = await db_1.db.query(`SELECT c.variant_key, c.crops, u.pseudo, u.avatar_pigment AS pig, p.url, p.delta_e
       FROM contributions c
       JOIN photos p ON p.id = c.photo_id
       JOIN app_users u ON u.id = c.user_id
       WHERE c.instance_id = $1 AND c.artwork_id = $2`, [id, a.id]);
        return reply.send({
    ...
    app.get('/:id/leaderboard', { onRequest: [app.authenticate] }, async (req, reply) => {
        ...
        const a = await (0, cycle_1.currentArtwork)();
        const { year, week } = (0, cycle_1.isoWeek)();
        const rows = await db_1.db.query(`SELECT u.id, u.pseudo, u.avatar_pigment AS pig,
       ...
       ORDER BY points DESC, u.created_at`, [userId, id, a?.id ?? '', year, week]);
        return reply.send({
...
async function importSharedPhotos(userId, instanceId, mode) {
    if (mode !== 'shared')
        return;
    const a = await (0, cycle_1.currentArtwork)();
    if (!a)
    ...
    await db_1.db.query(`INSERT INTO contributions (photo_id, instance_id, artwork_id, user_id, variant_key, crops)
     ...
     ON CONFLICT (instance_id, artwork_id, variant_key) DO NOTHING`, [userId, instanceId, a.id]);
}
