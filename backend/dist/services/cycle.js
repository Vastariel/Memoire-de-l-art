...
exports.artworkId = artworkId;
exports.currentArtwork = currentArtwork;
exports.activateCurrentWeek = activateCurrentWeek;
exports.revealCurrentWeek = revealCurrentWeek;
exports.publicArtwork = publicArtwork;
...
function artworkId({ year, week }) {
    return `w${year}-${String(week).padStart(2, '0')}`;
}
/** The week's artwork (active or revealed) for the given/current date, or null. */
async function currentArtwork(at = new Date()) {
    const { year, week } = isoWeek(at);
    const res = await db_1.db.query(`SELECT id, title_fr, title_en, artist, year_, description_fr, description_en,
...
async function activateCurrentWeek() {
    const { year, week } = isoWeek();
    await db_1.db.query(`UPDATE artworks SET status = 'active' WHERE status = 'planned' AND iso_year = $1 AND iso_week = $2`, [year, week]);
}
/** Sunday 23:59 UTC — resolve guesses, award bet points, reveal the artwork. */
async function revealCurrentWeek() {
    const a = await db_1.db.query(`SELECT id, title_fr, title_en FROM artworks WHERE status = 'active'
     ORDER BY iso_year DESC, iso_week DESC LIMIT 1`);
    if (a.rows.length === 0)
        return;
    const art = a.rows[0];
    await db_1.db.query(`UPDATE guesses SET correct = (LOWER(title_guess) = LOWER(COALESCE($2,''))
                                   OR LOWER(title_guess) = LOWER(COALESCE($3,'')))
     WHERE artwork_id = $1`, [art.id, art.title_fr, art.title_en]);
    // Award the day-based barème to each instance the correct guessers belong to.
    await db_1.db.query(`INSERT INTO scores (user_id, instance_id, iso_year, iso_week, points)
     ...
     DO UPDATE SET points = scores.points + EXCLUDED.points`, [art.id]);
    await db_1.db.query(`UPDATE artworks SET status = 'revealed' WHERE id = $1`, [art.id]);
}
/** Public-facing metadata is hidden until the artwork is revealed. */
function publicArtwork(a, locale) {
